  ...
  return (
    <span className="relative inline-flex">
      <button
        ...
        title={t("Gerar uma sugestão editorial com IA")}
      ...
      {status === "success" && result && (
        <span className="animate-fade-in absolute top-full right-0 z-30 mt-1 flex w-[min(420px,80vw)] flex-col gap-2 rounded-lg border border-teal/30 bg-surface-2 p-3 text-[11px] leading-relaxed text-ink shadow-xl">
          <strong className="text-teal">{mode === "ai" ? t("Sugestão gerada por IA") : t("Sugestão editorial local")}</strong>
          <span className="max-h-64 overflow-auto whitespace-pre-wrap">{t(result)}</span>
          <span className="flex justify-end gap-2">
            <button type="button" className="rounded border border-line px-2 py-1" onClick={async () => { await navigator.clipboard.writeText(result); setMessage("Copiado."); }}>{t("Copiar")}</button>
            <button type="button" className="rounded bg-teal px-2 py-1 font-semibold text-abyss" onClick={() => setStatus("idle")}>{t("Fechar")}</button>
          </span>
          {message && <small className="text-teal">{t(message)}</small>}
        </span>
      )}
    </span>
  );
